...
// ─── Widget countdown ─────────────────────────────────────────
function CountdownWidget({ diasDesbloqueados, color = '#a78bfa' }) {
  const { horas, minutos, segundos } = useCountdown();
  const proximoDia = diasDesbloqueados + 1;
  if (proximoDia > TOTAL) return null;
...
export default function DespertarDaysScreen({ startedAtISO, completedDays = [], onBack, onOpenDay }) {
  ...
  const diasDesbloqueados = getDiasDesbloqueados();
  ...
  const progresoPct = `${Math.max((diasCompletados.length / TOTAL) * 100, 0)}%`;
  ...
  return (
    <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
      ...
      <View style={styles.body}>
        ...
        {/* ── COUNTDOWN ── */}
        <CountdownWidget diasDesbloqueados={diasDesbloqueados} color="#a78bfa" />
...
const styles = StyleSheet.create({
  ...
  progressFill:   { backgroundColor: '#a78bfa', borderRadius: 8, height: 8, shadowColor: '#a78bfa', shadowOffset: { width: 0, height: 0 }, shadowOpacity: 0.5, shadowRadius: 4 },
  ...
  diasGrid:       { flexDirection: 'row', flexWrap: 'wrap', rowGap: 10, columnGap: 10 },
